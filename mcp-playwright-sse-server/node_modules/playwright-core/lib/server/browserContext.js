...
`);
  ...
  async storageState(indexedDB = false) {
    ...
    const collectScript = `(${storageScript.collect})(${utilityScriptSerializers.source}, (${import_builtins.ensureBuiltins})(globalThis), ${this._browser.options.name === "firefox"}, ${indexedDB})`;
    for (const page of this.pages()) {
    ...
    try {
      ...
      if (state.origins && state.origins.length) {
        ...
        for (const originState of state.origins) {
          ...
          await frame.evaluateExpression(`(${storageScript.restore})(${utilityScriptSerializers.source}, (${import_builtins.ensureBuiltins})(globalThis), ${JSON.stringify(originState)})`, { world: "utility" });
        }
        await page.close(internalMetadata);
      }
    } finally {
