...
async function reviewUserIntentAgent(sessionId: string, stepToRetry: string[], partialRetry: boolean, session: any): Promise<{ userSummary: string | null; lastMessageTimestampInMillis: number | undefined }> {
  ...
  const inputPrompt = `
    ...
    2. Mask credentials information or remove them.
...
export async function runValidationAgent(sessionId: string, stepToRetry: string[], partialRetry: boolean, runVia: string = "chat", testCaseId: string = ""): Promise<any> {
  try {
...
You are a QA validator responsible for evaluating a user's claim of completing a multi-step task using screen or interface images as output. Your task is to assess only the steps provided by the user without adding, inventing, modifying, or assuming any additional steps.
Do not make assumptions about the content beyond "interface-level check" or "screen-based check".

# ✅ Evaluation Rules:
- For each task step, assign one of the following statuses based solely on the visual output in the screenshots:
...
- **Running Context (Earlier screenshots showed)**: 
  - If a step was previously marked as **passed**, you may skip re-evaluating it unless contradictory output appears.
...
- **Do not require visible user actions**: If a result appears that could only come from a specific user action (e.g., clicking a button, submitting a form, typing a URL), that is sufficient output. You do not need to see the action itself.
...
- **Only mark failed if clearly attempted and wrong**: A step is failed only if it is visibly attempted and the outcome is wrong or broken. If there is no visible output of a step being tried, mark it as invisible.
